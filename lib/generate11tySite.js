...
const { writeCssVariables, transformKeys, CharCase } = require('./customStyles');
...
const generateOrUpdate11tySite = async (site) => {
  ...
  try {
    ...
    const styles = site.theme 
      ? removeObjNullValues(transformKeys(site.theme, CharCase.KEBAB))
      : { 'primary-brand-color': 'purple' }

		await buildStyles(eleventyContentPath, styles)
